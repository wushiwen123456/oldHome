<template>
	<view class="content">
		<view class="search-box">
			...
			<mSearch ref="mSearch" class="mSearch-input-box" @getFocus="getFocus" :mode="2" button="inside" :placeholder="defaultKeyword" @search="doSearch(false)" @input="inputChange" @confirm="doSearch(false)" v-model="keyword"></mSearch>
		...
		<view class="search-keyword" @touchstart="blur">
			<scroll-view class="keyword-list-box" v-show="isShowKeywordList" scroll-y>
				...
				<view class="l-others" v-if="!keywordList.length">
					暂无此类别哦，换个词试试吧
				</view>
			</scroll-view>
			<scroll-view class="keyword-box" v-show="!isShowKeywordList" scroll-y>
				<view class="keyword-block" v-if="oldKeywordList.length>0">
					...
					<view class="keyword">
						<view v-for="(keyword,index) in oldKeywordList" @tap="doInput(keyword)" :key="index">{{keyword}}</view>
					</view>
				</view>
				<view class="keyword-block">
					...
					<view class="keyword" v-if="forbid==''">
						<view v-for="(keyword,index) in hotKeywordList" @tap="doInput(keyword)" :key="index">{{keyword}}</view>
					</view>
	...
	export default {
		...
		methods: {
			...
			loadHotKeyword() {
				...
				this.hotKeywordList = ['测试', '鼠标', '显示器', '电脑主机', '蓝牙音箱', '笔记本电脑', '鼠标垫', 'USB', 'USB3.0'];
			}, 
			//监听输入
			inputChange(event) {
				//兼容引入组件时传入参数情况
...
<style>
	...
	.l-others{height: 100%; display: flex; justify-content: center;align-items: center; color:#333; opacity: .5; font-size: $uni-font-size-lg;}
</style>
